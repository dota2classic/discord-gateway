...
export class EngageNeededHandler implements IEventHandler<EngageNeededEvent> {
  ...
  private possibleOptions = [
    ...
    `Есть инфа от знающего человека, что на сервере скоро будет игра. После того, как челики стабилизируют ситуацию в IRL и придут с работы. Тогда и начнём катать. Онлайн поднимется и не будет падать, Вальв ничего не сможет сделать. Сейчас главное не бухтеть.`
  ];
