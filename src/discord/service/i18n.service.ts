...
\`!site\` - страницы сайта`;
  ...
  welcomeMessage() {
    return new MessageEmbed()
      .setDescription(`**Как начать играть в Dota 2 Classic 6.84c?**

1. Скачайте клиент Dota 2 Classic 6.84c на сайте https://dota2classic.ru/download
2. Авторизуйтесь на нашем сайте через Steam https://dota2classic.ru/api/v1/auth/steam
**3. Запустите Steam и войдите в свой аккаунт (в тот, через который авторизовались на нашем сайте).**
4. Запустите старый клиент через файл dota.exe, находящийся в папке Dota 6.84.
5. На странице поиска игры https://dota2classic.ru/queue выберите режим и нажмите *Искать игру*.  Если закрыть вкладку, поиск прекратится через минуту.
6. Когда игра будет найдена, на сайте появится и прозвучит уведомление для принятия игры. 
